...
function App() {
  ...
  const handleInput = () => {
    ...
    let askWeather = [
      "What is the weather in Kelowna?|Kelowna Weather|Weather Kelowna|kelowna weather|weather kelowna|weather Kelowna|Kelowna weather",
    ]; //adding the age-question
    let words12 = new RegExp(askWeather);
    if (words12.test(document.querySelector("#input").value)) {
      // if the input contains some question
      fetch('http://api.weatherstack.com/current?access_key=dd96a8ed4e3d4d3f30052e7496989c56&query=Kelowna', {
        method: 'GET'
      })
        ...
        .then(function (json) {
          ...
          getBotMessage.innerText = "Typing...";
          setTimeout(() => {
            getBotMessage.innerText = "Current Temprature: " + json.current.temperature;
            inputRef.value = ""; // clear the input
          }, 2000);
        });
    }
    ...
    let askJoke = [
      "Tell me a joke|tell me a joke|tell me a Joke|JOKE|joke|Joke|Tell me a JOKE",
    ]; //adding the age-question
    let words13 = new RegExp(askJoke);
    if (words13.test(document.querySelector("#input").value)) {
      ...
      fetch('https://official-joke-api.appspot.com/random_joke', {
        ...
        .then(function (json) {
          ...
          getBotMessage.innerText = "Typing...";
          setTimeout(() => {
            getBotMessage.innerText = json.setup + "\n" + json.punchline;
            inputRef.value = ""; // clear the input
          }, 2000);
        });
    }
